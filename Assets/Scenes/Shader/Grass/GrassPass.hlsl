...
void VertexMethod(VertexAttributes vertex, inout Varyings varyings, float4 timeOffset)
{
    ...
    float4 bendOffset = GetBendOffset(varyings.positionWS, vertex.color.r);
    bendOffset.y = -bendOffset.y;
    ...
    float3 resultOffset = lerp(windOffset, bendOffset, bendOffset.a);
    ...
    RecalculateWindNormals(input, varyings.normalWS, resultOffset);
...
void SurfaceMethod( Varyings input, inout Surface surface)
{
    ...
    // albedo.rgb = lerp(albedo.rgb,  _BendTint.rgb, saturate(bendingMask * sqrt(mask) * _BendTint.a));
    ...
    surface.Thickness = Remap(thickness, _ThicknessRemap.xy);
...
{   
    ...
    float3 translucency = Translucency(surface.Thickness,surface.Albedo, 
        inputData.bakedGI,input.normalWS,input.viewDirectionWS.xyz, mainLight);
